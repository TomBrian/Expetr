<template>
    <div class="home">
        ...
        <div class="flex py-2" style="justify-content:center;">
            <a class="navbar-brand" :href="location.origin"
                ><img
                    :src="'/images/logo.svg'"
                    class="v-lg-none"
                    alt=""
        ...
        <div class="organisation-details my-4 mx-3">
            <div
                class="organisation-name text-muted"
        ...
        <div class="display:flex;" v-if="organisation != null">
            <div
                class="notification-card shadow-sm p-3 text-center m-6"
                style="background-color:#17A2B8 !important;"
                v-if="requests.length > 0"
            >
                <router-link
                    to="/dashboard/advanced/membership"
                    class="nav-link"
                >
                    <div
                        class="font-weight-bolder text-muted"
                        style="margin:auto; color:white !important;"
                    >
                        There are {{ requests.length }} unattended membership
                        requests
                    </div>
                </router-link>
            </div>
            <div
                class="notification-card shadow-sm p-3 text-center my-2"
        ...
        <div class="pending shadow-sm my-3 p-2">
            ...
            <div class="content p-2">
                ...
                >
                    <div
                        class="card-header flex justify-content-between"
                        role="tab"
                        :id="`HeaderId${assignment.id}`"
                    >
                        <div>
                            <h5 class="mb-0">
                                <a
                                    data-toggle="collapse"
                                    :href="`#ContentId${assignment.id}`"
                                    aria-expanded="true"
                                    class="text-dark nav-link"
                                    :aria-controls="`ContentId${assignment.id}`"
                                >
                                    {{ assignment.title }}
                                </a>
                            </h5>
                        ...
                        <div>
                            <small class="text-muted"
                                >due
                                {{
                                    assignment.date_due | moment("from", "now")
                                }}</small
                            >
                        </div>
                    </div>
                    <div
                        :id="`ContentId${assignment.id}`"
                        class="collapse in"
                        role="tabpanel"
                        :aria-labelledby="`HeaderId${assignment.id}`"
                    >
                        <div class="card-body">
            ...
            <div class="events-card shadow-sm mx-3">
                ...
                <div class="content p-4" v-if="eventsAlmostDue != null">
                    ...
                    >
                        <div
                            class="card"
                            v-for="event in eventsAlmostDue"
                            :key="event.id"
                        >
                            <div
                                ...
                                role="tab"
                                :id="`HeaderId${event.id}`"
                            >
                                <div>
                                    <h5 class="mb-0">
                                        <a
                                            data-toggle="collapse"
                                            :href="`#ContentId${event.id}`"
                                            aria-expanded="true"
                                ...
                                <div>
                                    <small class="text-muted">{{
                                        event.date | moment("from", "now")
                                    }}</small>
                                </div>
                            </div>
                            <div
                                :id="`ContentId${event.id}`"
                                class="collapse in"
                                role="tabpanel"
                                :aria-labelledby="`HeaderId${event.id}`"
                            >
                                <div class="card-body">
                                    {{ event.description }}
                                </div>
...
export default {
    ...
    methods: {
        ...
        getJoinRequests: function() {
            axios
                .get(location.origin + "/api/get-all-join-requests")
                .then(res => {
        ...
        getNotifications: function() {
            axios
                .get(location.origin + "/api/get-unread-notifications")
                .then(res => res.data)
                .then(data => {
                    this.unreadNotifications = data;
                    axios
                        .get(location.origin + "/api/get-read-notifications")
                        .then(res => res.data)
                        .then(data => {
